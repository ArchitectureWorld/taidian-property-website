...
class StoryIntegrationTests(unittest.TestCase):
    ...
    def test_image_layers_are_local_and_complete(self):
        image_layers = re.findall(r'class="story-image(?: scene-layer)? story-image--[0-3]"', self.html)
    ...
    def test_concept_specific_visual_contract(self):
        self.assertIn('data-concept="community-digital-layer"', self.html)
        self.assertIn("真实社区数字图层", self.html)
        for token in ['service-overlay', 'scene-layer', 'scan-line']:
