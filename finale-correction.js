(() => {
  ...
  const text = '泰典物业';
  let width = 1, height = 1, ratio = 1, storyStart = 0, storyDistance = 1;
  let targetProgress = 0, displayProgress = 0, raf = 0, lastTime = 0;
  ...
  function punchText(centerX, centerY, fontSize, tracking) {
    ...
    for (const character of text) { ctx.fillText(character,x,centerY); x += ctx.measureText(character).width + tracking; }
  }
  function draw(progress) {
    ...
    if (cover > 0.0001) {
      ctx.globalCompositeOperation='source-over'; ctx.globalAlpha=cover; ctx.fillStyle=theme.bg; ctx.fillRect(0,0,width,height);
      ctx.globalCompositeOperation='destination-out'; ctx.globalAlpha=cover; punchText(width/2+focusOffset,height/2,fontSize,tracking);
  ...
  function render(now) {
    ...
    lastTime=now; displayProgress=damp(displayProgress,targetProgress,17,dt); draw(displayProgress);
    if (Math.abs(displayProgress-targetProgress)>0.00012) raf=requestAnimationFrame(render);
    else { displayProgress=targetProgress; draw(displayProgress); raf=0; }
  }
  function schedule(){ if(raf)return; lastTime=performance.now(); raf=requestAnimationFrame(render); }
  function onScroll(){ targetProgress=clamp((scrollY-storyStart)/storyDistance); schedule(); }
